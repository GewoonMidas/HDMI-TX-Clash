...
import Hdmi.I2CSlave
import Hdmi.Init

system0 :: Clock System -> Reset System -> Signal System (Vec 99 (Unsigned 8), Bool, Bool)
system0 clk arst = bundle (regFile,done,fault)
 where
  (dout,hostAck,busy,al,ackOut,i2cO) = i2c clk arst rst (pure True) (pure 19) start stop (pure False) write (pure True) (din) i2cI

  (start,stop,write,din,done,fault) = hdmiConfig clk arst enableGen (rst, (fmap not rst),hostAck,ackOut,al)

  (_,sclOen,_,sdaOen) = unbundle i2cO
  scl  = fmap bitCoerce sclOen
  i2cI = bundle (scl,sdaS)

  (sdaS,regFile) = unbundle (i2cSlave clk (bundle (scl, bitCoerce <$> sdaOen)))
